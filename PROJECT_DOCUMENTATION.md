...
| 停机坪 | 涨停后连续3日高开小涨 |
| 回踩年线 | 突破250日均线后回踩确认 |
| 突破平台 | 横盘平台后近期放量突破60日均线，且当前仍站稳均线之上 |
| 无大幅回撤 | 60日内无大幅回撤稳健上涨 |
...
因子实验室 / 选股验证的默认主模板建议使用 `m1_auto_ic_m12`；`m1_auto_ic_m15` 和 `m1_auto_ic_m16` 保留给约束实验与对照比较。

> 复盘建议：在因子实验室里做模板对比时，先固定同一批样本、同一数据窗口和披露滞后，再比较 `m1_auto_ic_m12`、`m1_auto_ic_m15`、`m1_auto_ic_m16` 的 Top−Bottom 与单调性；默认展示口径仍建议回到 `m1_auto_ic_m12`。
...
# 整体作业（包含所有数据处理，自动执行5个阶段），不带参数按最近交易日运行
python execute_daily_job.py

# 指定单个交易日（YYYY-MM-DD 或 YYYYMMDD）
python execute_daily_job.py 2024-01-15
```

> 注：仅支持**单个交易日**，且仅覆盖日期相关步骤（作业记账、数据新鲜度检查、
