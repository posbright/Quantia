...
graph TB
subgraph "Web 层"
A["GetBacktestConfigHandler<br/>GET /quantia/api/backtest/config"]
B["RunBacktestHandler<br/>GET /quantia/api/backtest/run"]
C["RunBatchBacktestHandler<br/>GET /quantia/api/backtest/batch"]
...
Client->>Config : GET /quantia/api/backtest/config
Config->>Core : 读取周期/策略/默认horizon
Core-->>Config : 返回配置
Config-->>Client : 配置JSON
Client->>Run : GET /quantia/api/backtest/run?code&strategy&period&start_date&end_date&checkpoints
Run->>Core : 计算买入价/各周期收益/策略命中/关键指标
Core-->>Run : 回测结果
Run-->>Client : 结果JSON
Client->>Batch : GET /quantia/api/backtest/batch?strategy&period&limit&horizons&success_days
Batch->>DB : 查询策略表/回测汇总
DB-->>Batch : 按日聚合统计
Batch-->>Client : 聚合结果JSON
Client->>Dash : GET /quantia/api/backtest/dashboard/*
