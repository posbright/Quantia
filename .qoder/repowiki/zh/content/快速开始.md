...
graph TB
subgraph "应用层"
WEB["Web服务<br/>quantia/web/web_service.py"]
FRONT["前端静态资源<br/>quantia/web/static"]
end
subgraph "业务层"
JOB["每日作业调度<br/>quantia/job/*.py"]
CORE["核心业务<br/>quantia/core/*"]
end
subgraph "数据与配置"
DB["MySQL/MariaDB"]
CONF_DB["数据库配置<br/>quantia/lib/database.py"]
CONF_PROXY["代理配置<br/>quantia/config/proxy.txt"]
CONF_COOKIE["Cookie配置<br/>quantia/config/eastmoney_cookie.txt"]
CONF_TRADE["交易配置<br/>quantia/config/trade_client.json"]
end
WEB --> CORE
WEB --> FRONT
JOB --> CORE
...
graph TB
subgraph "Docker编排"
APP["应用容器<br/>posbright/Quantia:latest"]
