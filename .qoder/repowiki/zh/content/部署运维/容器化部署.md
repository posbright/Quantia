...
subgraph "数据库层"
DB["MySQL 8.0<br/>容器 quantiadb"]
V1["数据卷 QUANTIA_db_data"]
end
subgraph "应用层"
APP["Quantia 应用容器 quantia-app"]
S["Supervisor 进程管理"]
W["Web 服务进程"]
J["Job 任务进程"]
C["Cron 定时进程"]
V2["日志卷 QUANTIA_logs"]
V3["缓存卷 QUANTIA_cache"]
end
DB --- V1
APP --- S
S --> W
...
participant DB as "quantiadb"
participant APP as "quantia-app"
U->>DC : 启动编排
DC->>DB : 拉起MySQL容器
DB-->>DC : 健康检查通过
DC->>APP : 构建并启动应用容器
...
```mermaid
flowchart LR
A["应用容器 quantia-app"] --> B["远程数据库<br/>通过环境变量配置"]
A --> C["日志卷 QUANTIA_logs"]
A --> D["缓存卷 QUANTIA_cache"]
```
...
IMG --> SVC_APP["应用服务 quantia-app"]
IMG --> SVC_DB["数据库服务 quantiadb"]
