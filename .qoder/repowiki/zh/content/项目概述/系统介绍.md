...
FE->>API : GET /quantia/api/backtest/config
API->>BT : GetBacktestConfigHandler
BT-->>FE : periods, strategies, horizons
FE->>API : GET /quantia/api/backtest/run?code=...&strategy=...
API->>BT : RunBacktestHandler
BT->>DB : 读取策略表/指标表
BT-->>FE : 单股回测结果
FE->>API : GET /quantia/api/backtest/dashboard/overview
...
```mermaid
graph LR
EXEC[job/execute_daily_job.py] --> DB[quantia/lib/database.py]
