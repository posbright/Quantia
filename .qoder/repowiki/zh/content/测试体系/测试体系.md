...
subgraph "前端测试"
VITEST["Vitest 配置<br/>quantia/fontWeb/vitest.config.ts"]
SETUP["全局设置<br/>quantia/fontWeb/tests/setup.ts"]
PKG["脚本与依赖<br/>quantia/fontWeb/package.json"]
API["API Mock 测试<br/>quantia/fontWeb/tests/api/handlers.test.ts"]
STORE["状态管理测试<br/>quantia/fontWeb/tests/stores/stock.test.ts"]
VIEW["组件渲染测试<br/>quantia/fontWeb/tests/views/StockData.test.ts"]
end
PY --> CONF
PY --> BACKTEST
PY --> DBTEST
...
T->>S : "监听 handlers"
T->>AX : "GET /quantia/api_data?name=..."
AX->>S : "请求拦截"
S->>H : "转发至后端"
H-->>S : "返回数据"
S-->>AX : "返回响应"
