...
from sqlalchemy import DATE, VARCHAR, FLOAT, BIGINT, SmallInteger, DATETIME, Integer, TEXT, JSON
from sqlalchemy.dialects.mysql import BIT
import talib as tl
from quantia.core.strategy import enter
from quantia.core.strategy import turtle_trade
from quantia.core.strategy import climax_limitdown
from quantia.core.strategy import low_atr
from quantia.core.strategy import backtrace_ma250
from quantia.core.strategy import breakthrough_platform
...
# 基金经理经验（P4 弱因子，来源 akshare.fund_manager_em）。主键 (code, manager)。
# 一只基金可有多位经理 → 多行；tenure_days=经理累计从业天数、total_aum=经理在管总规模(亿)、
# best_return=经理现任基金最佳回报(%)、fund_count=经理在管基金数（抓取时按全表统计，供"一拖多"提示）。
# 仅"经理经验弱因子"展示用，不硬拦截、不进入 TimingScore（蓝图 §9.2 / P4）。
TABLE_CN_FUND_MANAGER = {'name': 'cn_fund_manager', 'cn': '基金经理经验',
                         'columns': {'code': {'type': VARCHAR(6, _COLLATE), 'cn': '基金代码', 'size': 70},
                                     'manager': {'type': VARCHAR(60, _COLLATE), 'cn': '经理姓名', 'size': 120},
                                     'company': {'type': VARCHAR(60, _COLLATE), 'cn': '所属公司', 'size': 160},
                                     'tenure_days': {'type': Integer, 'cn': '累计从业天数', 'size': 100},
                                     'total_aum': {'type': FLOAT, 'cn': '在管总规模(亿)', 'size': 110},
                                     'best_return': {'type': FLOAT, 'cn': '最佳回报(%)', 'size': 100},
...
# 专利护城河数据表（运行时由 stock_patent_crawler / aggregate_patent_data 建表，
# 此处登记元数据作为单一事实来源，见 AGENTS.md 规则 2）。
TABLE_CN_STOCK_PATENT_INFO = {'name': 'cn_stock_patent_info', 'cn': '上市公司专利公告原始数据',
                     'columns': {'id': {'type': BIGINT, 'cn': '自增ID', 'size': 0},
                                 'code': {'type': VARCHAR(10, _COLLATE), 'cn': '股票代码', 'size': 60},
                                 'name': {'type': VARCHAR(64, _COLLATE), 'cn': '名称', 'size': 120},
                                 'title': {'type': VARCHAR(500, _COLLATE), 'cn': '公告标题', 'size': 200},
                                 'announcement_id': {'type': VARCHAR(64, _COLLATE), 'cn': '公告ID', 'size': 0},
                                 'announcement_date': {'type': DATE, 'cn': '公告日期', 'size': 90},
                                 'category': {'type': VARCHAR(64, _COLLATE), 'cn': '公告类型', 'size': 90},
                                 'patent_type': {'type': VARCHAR(32, _COLLATE), 'cn': '专利类型', 'size': 90},
                                 'patent_count': {'type': Integer, 'cn': '涉及专利数量', 'size': 80},
                                 'url': {'type': VARCHAR(500, _COLLATE), 'cn': '公告链接', 'size': 0},
                                 'summary': {'type': TEXT, 'cn': '公告摘要', 'size': 200},
                                 'created_at': {'type': DATETIME, 'cn': '入库时间', 'size': 0}}}

TABLE_CN_STOCK_PATENTS = {'name': 'cn_stock_patents', 'cn': '上市公司专利数据(护城河量化)',
                     'columns': {'code': {'type': VARCHAR(10, _COLLATE), 'cn': '股票代码', 'size': 60},
                                 'year': {'type': Integer, 'cn': '统计年度', 'size': 60},
                                 'invention_patents': {'type': Integer, 'cn': '发明专利数', 'size': 80},
                                 'utility_patents': {'type': Integer, 'cn': '实用新型专利数', 'size': 90},
                                 'design_patents': {'type': Integer, 'cn': '外观设计专利数', 'size': 90},
                                 'total_patents': {'type': Integer, 'cn': '专利总数', 'size': 80},
                                 'new_patents_year': {'type': Integer, 'cn': '当年新增专利', 'size': 90},
                                 'patent_yoy': {'type': FLOAT, 'cn': '专利同比增长率(%)', 'size': 90},
                                 'invention_ratio': {'type': FLOAT, 'cn': '发明专利占比(%)', 'size': 90},
                                 'patent_quality_score': {'type': SmallInteger, 'cn': '专利含金量评分(0-100)', 'size': 100},
                                 'avg_citation_count': {'type': FLOAT, 'cn': '平均被引用次数', 'size': 90},
                                 'pct_international': {'type': Integer, 'cn': 'PCT国际专利数量', 'size': 90},
                                 'core_patents': {'type': Integer, 'cn': '核心专利数', 'size': 80},
                                 'patent_maintenance_rate': {'type': FLOAT, 'cn': '专利维持率(%)', 'size': 90},
                                 'ipc_primary': {'type': VARCHAR(20, _COLLATE), 'cn': '主IPC分类号', 'size': 80},
                                 'ipc_primary_desc': {'type': VARCHAR(100, _COLLATE), 'cn': '主IPC中文描述', 'size': 120},
                                 'ipc_distribution': {'type': JSON, 'cn': 'IPC分布', 'size': 0},
                                 'tech_domain': {'type': VARCHAR(50, _COLLATE), 'cn': '技术领域归类', 'size': 100},
                                 'trend_5y': {'type': JSON, 'cn': '近5年申请数列表', 'size': 0},
                                 'trend_5y_cagr': {'type': FLOAT, 'cn': '5年专利申请复合增长率(%)', 'size': 100},
                                 'trend_direction': {'type': VARCHAR(20, _COLLATE), 'cn': '趋势方向', 'size': 90},
                                 'rd_staff_count': {'type': Integer, 'cn': '研发人员数量', 'size': 90},
                                 'rd_staff_ratio': {'type': FLOAT, 'cn': '研发人员占比(%)', 'size': 90},
                                 'key_tech_desc': {'type': TEXT, 'cn': '核心技术描述', 'size': 200},
                                 'competitive_position': {'type': VARCHAR(200, _COLLATE), 'cn': '行业专利排名描述', 'size': 150},
                                 'data_source': {'type': VARCHAR(20, _COLLATE), 'cn': '数据来源', 'size': 90},
                                 'source_detail': {'type': JSON, 'cn': '来源明细', 'size': 0},
                                 'confidence_score': {'type': SmallInteger, 'cn': '数据可信度(0-100)', 'size': 90},
                                 'created_at': {'type': DATETIME, 'cn': '首次入库时间', 'size': 0},
                                 'updated_at': {'type': DATETIME, 'cn': '更新时间', 'size': 0}}}

# 公司概况（东方财富 F10：经营范围 / 主营构成明细 / 经营评述）。单行/股，主键 code。
# 数据季度/年度级稳定，由独立慢 job (fetch_company_profile_job) 低频抓取。mainop 存
# JSON 字符串（最新报告期的按行业/产品/地区拆分明细列表），供个股详情「公司概况」卡展示。
TABLE_CN_STOCK_COMPANY_PROFILE = {'name': 'cn_stock_company_profile', 'cn': '公司概况(经营范围/主营构成/经营评述)',
                     'columns': {'code': {'type': VARCHAR(6, _COLLATE), 'cn': '股票代码', 'size': 60},
                                 'report_date': {'type': DATE, 'cn': '报告期', 'size': 90},
                                 'business_scope': {'type': TEXT, 'cn': '经营范围', 'size': 0},
                                 'business_review': {'type': TEXT, 'cn': '经营评述', 'size': 0},
                                 'mainop': {'type': TEXT, 'cn': '主营构成明细(JSON)', 'size': 0},
                                 'update_date': {'type': DATE, 'cn': '更新日期', 'size': 90}}}
