...
_CRAWL_DELAY_EM_MIN = _cfg.get_float('QUANTIA_CRAWL_DELAY_EM_MIN', 0.2)
_CRAWL_DELAY_EM_MAX = _cfg.get_float('QUANTIA_CRAWL_DELAY_EM_MAX', 0.5)
...
def stock_zh_a_hist(
    ...
    # 当调度层已有限流（update_all_caches）时可通过 QUANTIA_CRAWL_DELAY_ENABLED=0 关闭
    if _CRAWL_DELAY_ENABLED:
        time.sleep(random.uniform(_CRAWL_DELAY_EM_MIN, _CRAWL_DELAY_EM_MAX))
    r =  fetcher.make_request(url, params=params)
    data_json = r.json()
