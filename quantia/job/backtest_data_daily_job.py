...
分层设计（按信号距今自然日）：
  Tier 1 (任意年龄): rate_1 IS NULL → 全新未处理记录          [LIMIT 防超时, date ASC]
  Tier 2 (7~27天):   rate_5 IS NULL AND rate_1 IS NOT NULL → 5日里程碑  [LIMIT 防超时, date ASC]
  Tier 3 (28~139天): rate_20 IS NULL AND rate_1 IS NOT NULL → 20日里程碑 [LIMIT 防超时, date ASC]
  Tier 4 (140+天):   rate_100 IS NULL AND rate_1 IS NOT NULL → 最终完成   [LIMIT 300, date ASC]

每条记录一生最多经过 4 次处理（vs 旧方案 ~100 次），日处理量降低 ~95%。

⚠️ 所有 tier 都必须限批（LIMIT + ORDER BY date ASC，最老的先处理）：
某些高选择性策略（如 cn_stock_strategy_low_atr 每日选 ~1800 只）会累积
数万条待回测记录。若 tier 1/2/3 无 LIMIT，单次子进程需处理 10万+ 条
（每条一次 gzip 缓存读取），在 ≤2GB 服务器上 7200s 内无法完成 → 子进程超时
→ 积压永远清不掉的恶性循环。限批后每次有界推进，积压按 date ASC 逐日排空。
可通过 QUANTIA_BACKTEST_TIER_LIMIT 调整每个 tier 单次处理上限。
"""
# 每个 tier 单次（每张表）处理记录上限，防止大积压拖垮子进程超时预算。
# 默认 5000：低选择性策略日增量 < 上限即可逐日排空积压。
_TIER_BATCH_LIMIT = _cfg.get_int('QUANTIA_BACKTEST_TIER_LIMIT', 5000)

# (target_col, prereq_col, min_age_days, max_age_days, limit)
_MILESTONE_TIERS = [
    ('rate_1',   None,     None, None, _TIER_BATCH_LIMIT),  # 全新记录
    ('rate_5',   'rate_1', 7,    27,   _TIER_BATCH_LIMIT),  # 5日里程碑
    ('rate_20',  'rate_1', 28,   139,  _TIER_BATCH_LIMIT),  # 20日里程碑
