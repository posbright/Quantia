...
def _normalize_db_charset(raw: str) -> str:
    """规范化并校验数据库字符集配置。

    根因防护：PyMySQL 建连时执行 ``charset_by_name(charset).encoding``，若 charset
    名不被识别（返回 None）会在建立 socket 之前就抛 ``AttributeError: 'NoneType'
    object has no attribute 'encoding'``，导致 get_connection / SQLAlchemy engine
    全部失败、定时任务整体崩溃。常见误配：
      - 把 collation 当 charset（utf8mb4_general_ci）
      - 带连字符（utf-8，应为 utf8）
      - .env / shell 导出残留引号或首尾空白（"utf8mb4" / 'utf8mb4 '）
    这里在配置边界统一清洗（去引号/空白）并校验，非法值回退到 utf8mb4 并告警，
    避免一处配置笔误拖垮整个采集/分析链路。
    """
    ...
    if not cleaned:
        return 'utf8mb4'
    if charset_by_name(cleaned) is None:
        logging.warning(
            "QUANTIA_DB_CHARSET=%r 不是 PyMySQL 可识别的字符集名，已回退为 'utf8mb4'。"
            "（charset 应为 utf8mb4，而非 collation utf8mb4_general_ci；不要带引号/连字符/空格）",
            raw,
        )
        return 'utf8mb4'
    return cleaned
...
db_charset = _normalize_db_charset(_cfg.get_str('QUANTIA_DB_CHARSET', 'utf8mb4'))  # 数据库字符集
