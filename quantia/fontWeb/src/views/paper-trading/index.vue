<template>
  <div class="paper-trading">
    ...
    <StockDialog
      ...
      :trades="detailData?.trades || []"
      :positions="detailData?.positions || []"
      :anchor-date="posHistDate || detailData?.info?.last_run_date || ''"
      :is-mobile="isMobile"
      :format-money-full="formatMoneyFull"
    />
...
import {
  ...
  deletePaperTrading, getPortfolioBacktestList, updatePaperTrading,
...
const BUILTIN_STRATEGY_DESC: Record<string, { buy: string; sell: string }> = {
  ...
  '低ATR成长': {
    buy: '上市满 120 日，近 20 日平均波动 ≤ 4%（低波动，寻优默认），价格振幅、回撤与成交额达标；区间涨幅/上涨天数占比为可选过滤。',
    sell: '趋势走坏或触发止损时卖出。',
  },
  '高而窄的旗形': {
...
function openPaperStock(row: any) {
  const code = String(row?.code || '').trim()
  if (!code) return
