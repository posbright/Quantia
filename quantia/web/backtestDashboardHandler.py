...
def _get_table_trade_date_count(table_name: str, start_date: str, end_date: str):
    ...
    df = mdb.read_sql_ro(sql, params=(start_date, end_date))
    try:
...
class StrategyDetailHandler(webBase.BaseHandler, ABC):
    ...
    def _handle(self):
        ...
        cnt_df = mdb.read_sql_ro(sql_cnt, params=(start_date, end_date))
...
class TradePairHandler(webBase.BaseHandler, ABC):
    ...
    def _handle(self):
        ...
        sql_buy = f"""SELECT `date`, `code`, `name`
                      ...
                      ORDER BY `date` DESC
                      LIMIT {offset}, {page_size_i}"""
        buy_df = mdb.read_sql_ro(sql_buy, params=(start_date, end_date))
        ...
        sell_df = mdb.read_sql_ro(sql_sell, params=tuple(sell_params))
