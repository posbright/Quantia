...
class SelectionScoreListHandler(webBase.BaseHandler):
    ...
    def get(self):
        try:
            ...
            total_df = pd.read_sql(count_sql, con=mdb.engine_ro(), params=tuple(params))
            total = int(total_df.iloc[0]['cnt']) if not total_df.empty else 0

            if need_python_sort:
                df_all = pd.read_sql(data_sql_all, con=mdb.engine_ro(), params=tuple(params))
                ...
                df = pd.read_sql(data_sql, con=mdb.engine_ro(), params=tuple(data_params))
...
class SelectionScoreDetailHandler(webBase.BaseHandler):
    ...
    def get(self):
        try:
            ...
            df = pd.read_sql(sql, con=mdb.engine_ro(), params=params)
            if df.empty:
                _write_json(self, _with_contract({
                    'item': None,
...
class SelectionScoreTopHandler(webBase.BaseHandler):
    ...
    def get(self):
        try:
            ...
            df = pd.read_sql(sql, con=mdb.engine_ro(), params=tuple(list(params) + [n]))
