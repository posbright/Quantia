...
| `/quantia/api/selection/score/detail` | GET | `code`, `date?` | 单股评分详情（七维分 + 标签 + 风险标记 + 排名可比标记，雷达图用） |
| `/quantia/api/selection/score/top` | GET | `n=20`, `template?` | 全市场 TopN（**固定按 `quality_score` 排序**；`template` 可传入但会被忽略，保证可比） |
...
- `detail/top` 对齐：`detail` 顶层返回 `display_score_field` 与 `view_score_active=false`；`top` 顶层返回 `display_score_field`、`view_score_active=false`、`template_ignored=true`（该接口固定按 `quality_score` 排序）。
- M3.6 模板元信息：`list/industries` 返回 `template_requested`、`template_effective`、`template_fallback`；其中 `template_fallback=true` 表示请求模板不可用且已回退到 `balanced`。`top` 也返回 `template_requested` 与 `template_effective=balanced`（并保留 `template_ignored=true`）。
- M3.7 排序元信息（list）：返回 `sort_requested`、`sort_effective`、`sort_fallback`；当排序键不在白名单时自动回退 `total_score` 且 `sort_fallback=true`。
- M3.8 契约版本标识：四个接口统一返回 `api_contract_version`（当前 `m3.8`）与 `api_contract_endpoint`（`list/detail/industries/top`），用于前端灰度、日志检索与问题追踪。
- M3.9 日期元信息：`list/detail/industries/top` 统一返回 `date_requested` 与 `date_effective`；当显式传入 `date` 参数时，响应 `date/date_effective` 与请求日期保持一致，不再回传全表最新日期。
- M3.10 技术面维度落地与标签生成（修复 `score_technical` 恒为 0、`tags`/`risk_flags` 恒为空）：
  - **技术面打分**：`core.selection_scoring.compute_technical_q` 基于 `cn_stock_selection` 的 BIT 信号列（MACD/KDJ 金叉、均线多头/突破、放量与看涨/看跌 K 线形态等）按 §4.4 加权计数得净看涨强度，再走稳健 Z → logistic 归一，落入 `score_technical`。此前 `DIMENSION_FAMILIES` 缺少 `technical` 键导致该维度恒缺失（写库为 NULL、前端雷达塌缩为 0）。无任何技术信号列时仍返回 `None`（不污染加权，保持向后兼容）。
  - **亮点/风险标签**：`build_daily_selection_scores` 在产出 `score_*` 后生成 `tags`（评级 S/A、各维度 ≥70、行业龙头/前三）与 `risk_flags`（各维度 ≤30、数据完整度 <0.5），不再硬编码为空数组；行业变更时仍追加 `rank_change_not_comparable`。
  - **行业宫格热度排序**：`industries` 接口每个行业新增 `heat_score = 均分 × ln(1+成分股数)`，行业卡片改按热度降序，避免少量成分股的小众行业凭高均分占据"行业宫格"，前端宫格展示数提升至 16。
  - **个股详情跳转**：`/selection/detail/:code` 页头股票名称/代码可点击跳转 `/indicator/detail`（K 线技术指标详情），便于下钻查看更细数据。
  - 以上改动需重跑 `selection_score_job` 回填 `cn_stock_selection_score`（`score_technical`/`tags`/`risk_flags`）方能在前端生效。
...
- `industries` 返回 `items`：每个行业含 `stock_count`、`avg_total_score`、`avg_quality_score`、`leader_*`、`comparable_ratio`、`non_comparable_count`、`heat_score`（=均分×ln(1+成分股数)，行业卡片默认按其降序）。
...
| `/selection/industry/:name` | 行业详情榜 | 单行业内股票评分排序 |
...
### 7.3 行业详情榜（/selection/industry/:name）
...
> M4 进展（2026-06-02）：`quantia/fontWeb/src/views/selection/all.vue` 已落地并接入 `/quantia/api/selection/score/list|industries|top`，实现概览 KPI、模板/排序筛选、Top 榜、行业宫格与评分表分页；页面已消费 M3 契约元信息（display/template/sort/date/contract version）。

> M5 进展（2026-06-02）：已新增 `quantia/fontWeb/src/views/selection/industry.vue` 与 `quantia/fontWeb/src/views/selection/detail.vue`，并在路由注册 `/selection/industry/:name`、`/selection/detail/:code`。M4 首页行业卡片与股票行可钻取到行业榜和个股详情；个股页提供七维雷达、优势/短板归因、亮点与风险标签展示。

> M6 进展（2026-06-02）：后端 `quantia/core/selection_scoring.py` 已增加官方模板 `value/growth/technical/financial` 并接入模板解析；`web/selectionScoreHandler.py` 在 `list/industries/detail/top` 响应新增 `template_weights` 元信息（与 `template_effective` 配套）。前端 `quantia/fontWeb/src/views/selection/all.vue` 与 `quantia/fontWeb/src/views/selection/industry.vue` 新增模板画像区，展示模板回退提示与权重重点维度，保证“模板切换是否真实生效”可视化可验证。
