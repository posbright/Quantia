...
| `/quantia/api/selection/score/list` | GET | `industry?`, `date?`, `rating?`, `min_quality?`, `template?`, `sort`, `page`, `page_size` | 股票评分榜（分页） |
...
- `template`（list/industries）：仅对**当前结果**按存储的 7 维度分**实时重加权**排序（P11），不改变 `quality_score`/`rating`。
