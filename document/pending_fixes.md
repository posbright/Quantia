...
- 未做（pending 后续 PR-11b 完整版）：底部 sheet 形态的 AgentPicker / ModelPicker（需新建 `el-drawer direction="btt"`）、`useVirtualKeyboard` 软键盘自适应、`useBodyScrollLock`（el-drawer 默认已 lock-scroll，暂无需）。
